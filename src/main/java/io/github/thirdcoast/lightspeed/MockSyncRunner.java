...
public class MockSyncRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        try (var input = getClass().getResourceAsStream("/fixtures/daily-report.json")) {
            ...
            DailySalesSummaryProvider provider = date -> new LightspeedMapper().map(withDate(report, date), "USD");
            ...
            reconciliation.results().forEach((date, result) -> log.info("Mock Lightspeed {}: {} journal entry {}", date, result.action(), result.quickBooksId()));
            reconciliation.failures().forEach((date, failure) -> log.error("Mock Lightspeed {} failed: {}", date, failure));
        }
    }
    private LightspeedDailyReport withDate(LightspeedDailyReport r, LocalDate date) {
        return new LightspeedDailyReport(r.shopId(), date, r.grossSales(), r.discounts(), r.returns(), r.tax(), r.tips(),
                r.giftCardsSold(), r.giftCardsRedeemed(), r.fees(), r.payments());
    }
}
